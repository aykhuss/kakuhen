...
struct Options {
  ...
  std::optional<bool> frozen;  //!< Whether to freeze grids (disable adaptation + data collection).
  ...
  inline void set(const Options& opts) {
    ...
    if (opts.frozen) frozen = *opts.frozen;
  ...
  struct keys {
    ...
    static constexpr OptionKey<&Options::frozen> frozen{};        //!< Key for `frozen` option.
